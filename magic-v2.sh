...
install_postgresql() {
    ...
    echo -e "   ${BOLD}Change this line:${NC}"
    echo -e "   ${RED}local   all             postgres                                peer${NC}"
    echo -e "   ${BOLD}To this:${NC}"
    echo -e "   ${GREEN}local   all             postgres                                scram-sha-256${NC}"
    echo -e "   ${BOLD}Ensure other entries use scram-sha-256:${NC}"
    ...
    echo -e "   ${CYAN}# Use 'localhost' for local only, or '*' for all interfaces${NC}\n"

    echo -e "${YELLOW}4. Restart PostgreSQL service:${NC}"
    echo -e "   ${CYAN}sudo systemctl restart postgresql${NC}"
    echo -e "   ${CYAN}sudo systemctl status postgresql${NC} # Verify it's running\n"

    echo -e "${YELLOW}5. Test connection methods:${NC}"
    echo -e "   ${BOLD}Initial test (should work immediately):${NC}"
    echo -e "   ${CYAN}sudo -u postgres psql -c 'SELECT version();'${NC}"
    echo -e "   ${BOLD}Password authentication test:${NC}"
    echo -e "   ${CYAN}psql -U postgres -W${NC}"
    echo -e "   ${CYAN}psql -h localhost -U postgres -W${NC} # If socket connection fails\n"

    echo -e "${YELLOW}6. Essential PostgreSQL commands:${NC}"
    echo -e "   ${BOLD}Connection commands:${NC}"
    echo -e "   ${CYAN}psql -U postgres -W${NC}                    # Connect with password"
    ...
    echo -e "   ${CYAN}CREATE USER 'your_username' WITH ENCRYPTED PASSWORD 'your_secure_password';${NC}"
    echo -e "   ${CYAN}GRANT ALL PRIVILEGES ON DATABASE myapp TO 'your_username';${NC}"
    echo -e "   ${CYAN}\\q${NC}\n"
